...
public class TaiKhoanBUS {
    ...
    public void datLaiMatKhau(String ma, String tenDangNhap) {
        ...
        if (flag) {
            new MyDialog("Đặt lại MK thành công! Mật khẩu mới là: " + tenDangNhap, MyDialog.SUCCESS_DIALOG);
        } else {
    ...
    public boolean themTaiKhoan(String ma, String tenDangNhap, String quyen) {
        ...
        if (tenDangNhap.trim().equals("")) {
            new MyDialog("Không để trống tên đăng nhập!", MyDialog.ERROR_DIALOG);
            return false;
        }
        if (kiemTraTrungTenDangNhap(tenDangNhap)) {
            MyDialog dlg = new MyDialog("Tên đăng nhập đã trùng! Có thể tài khoản bị khoá, thực hiện mở khoá?", MyDialog.WARNING_DIALOG);
    ...
    public boolean doiMatKhau(String matKhauCu, String matKhauMoi, String nhapLaiMatKhau) {
        ...
        if (flag) {
            ...
            new MyDialog("Mật khẩu cũ sai!", MyDialog.ERROR_DIALOG);
        }
        return flag;
    }
